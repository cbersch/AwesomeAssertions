...
public class EnumerableWithIndexValueFormatter : IValueFormatter
{
    /// <summary>
    ...
    {
        return value is EnumerableValue;
    }

    public void Format(object value, FormattedObjectGraph formattedGraph, FormattingContext context, FormatChild formatChild)
    {
        int relevantIndex = ((EnumerableValue)value).RelevantIndex;
        IEnumerable<object> collection = ((EnumerableValue)value).Value.Cast<object>();

        int initialIndex = Math.Max(relevantIndex - (MaxItems / 2), -1);
        using var iterator = new Iterator<object>(collection, MaxItems, initialIndex);
        ...
        while (iterator.MoveNext())
        {
            ...
            else
            {
                ...
                string moreItemsMessage = value is ICollection c ? $"…{c.Count - MaxItems - initialIndex + 1} more…" : "…more…";
            ...
            if (iterator.IsLast)
            {
                if (initialIndex > -1)
                {
                    iteratorGraph.AddStartingLineOrFragment($"…{initialIndex + 2} more…, ");
